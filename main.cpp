...
int main(int ac,char* av[])
{
	...
	("help","Produce help message.")// $ ./mikukawaii --miku
	...
	("dl,-l",po::value<std::string>(),"Load dynamic link library")
	;
	...
	{
	    void *handle;
    	    const char* dl_file = vm["dl"].as<std::string>();
	    
	    handle = dlopen(dl_file,RTLD_LAZY);
	    if(!handle)
	    {
	    ...
	    typedef void (*printmess_t)();
	    
	    dlerror();
	    printmess_t printmess = (printmess_t) dlsym(handle, "printmess");
	    const char *dlsym_error = dl_error();
	    if(dlsym_error)
	    {
		std::cerr << "Cannot load symbol 'printmess': " << dlsym_error << std::endl;
		dlclose(handle);
		return 1;
	    }
